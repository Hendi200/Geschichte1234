...
import { ChevronDown, MapPin, Crown, Swords, Ship, ShieldCheck, Pickaxe, Star, AlertTriangle, Crown as CrownLogo, Bomb, Globe, ArrowUp, ArrowRight, Sun, Moon } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Scroll Spy & Logo Visibility
  useEffect(() => {
    const handleScroll = () => {
      const scrollY = window.scrollY;
      const windowHeight = window.innerHeight;

      // Show Logo if past Hero
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden transition-colors duration-500 bg-[#f2f2f7] dark:bg-black text-[#1c1c1e] dark:text-[#f5f5f7] selection:bg-ottoman-gold selection:text-white pb-20">
      ...
      <button 
        onClick={toggleTheme}
        className="fixed top-4 right-4 md:top-6 md:right-6 z-50 w-10 h-10 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors bg-white/50 dark:bg-black/50 rounded-full backdrop-blur-md border border-black/5 dark:border-white/10 shadow-lg hover:scale-110 active:scale-95"
      ...
      <button 
         ...
         className={`fixed top-4 left-4 md:top-6 md:left-20 z-50 flex items-center gap-3 transition-all duration-500 ${showLogo ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}
      >
        <div className="w-10 h-10 bg-black dark:bg-white text-white dark:text-black rounded-full flex items-center justify-center shadow-lg">
           <CrownLogo size={20} />
        </div>
        <span className="font-serif font-bold text-lg hidden md:block backdrop-blur-md px-3 py-1 rounded-full bg-white/30 dark:bg-black/30">Osmanisches Reich</span>
      </button>
      ...
      <div id="hero" className="relative h-screen min-h-[500px] w-full flex flex-col items-center justify-center overflow-hidden bg-black">
        ...
        {/* Adjusted padding and gap to prevent "shifted" feel */}
        <div className="relative z-10 text-center px-4 md:px-24 max-w-7xl mx-auto flex flex-col items-center gap-2 md:gap-4">
            <h1 className="text-[12vw] md:text-[8rem] font-serif font-bold text-white tracking-tighter leading-none drop-shadow-2xl mix-blend-overlay">
        ...
        <button 
          ...
          className="absolute bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 text-white/50 hover:text-white transition-colors animate-bounce"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 max-w-6xl mx-auto">
           <BentoCard title="Der Erste Weltkrieg" icon={AlertTriangle} image={getAsset('fall_bg')} className="min-h-[400px] md:min-h-[500px]">
              <div className="h-full flex flex-col justify-center">
                <GlassBox>
                    <h4 className="text-xl md:text-2xl font-bold text-white mb-2 md:mb-4">Das letzte Aufbäumen</h4>
